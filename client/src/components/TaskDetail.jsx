...
import { X, CalendarClock, Flag, Hash, Trash2 } from "lucide-react";

export default function TaskDetail({ task, onClose, onUpdate, onDelete }) {
  const [title, setTitle] = useState(task.title);

  // Sync state if a different task is selected
  useEffect(() => {
    setTitle(task.title);
  }, [task]);
  ...
  return (
    <motion.aside
      ...
      className="w-80 lg:w-96 border-l border-neutral-200 dark:border-neutral-800/50 bg-neutral-50/30 dark:bg-neutral-900/10 flex flex-col h-full shrink-0"
    >
      {/* Header */}
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-[100px_1fr] items-center gap-y-4 text-sm">
            ...
            <select
              ...
              className="bg-transparent text-neutral-900 dark:text-neutral-200 font-medium outline-none cursor-pointer"
            >
